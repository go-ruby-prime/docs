...
Contributions are welcome. `go-ruby-prime/prime` is built to a small set of non-negotiable
...
- **Differential testing against MRI.** Correctness is defined by reference
  Ruby. A corpus is computed through both `ruby` (`Prime.prime?`, `Prime.take`,
  `Prime.prime_division`, …) and this library and the results are compared
  byte-for-byte, not approximated from memory.
- **Pure Go, cgo disabled.** The whole point is a single static binary with no C
  toolchain. Code must build with `CGO_ENABLED=0` (the only dependency is
  `math/big`). If a feature seems to need C, it needs a pure-Go path instead.
...
The library is in
[`github.com/go-ruby-prime/prime`](https://github.com/go-ruby-prime/prime). This documentation site is in
[`github.com/go-ruby-prime/docs`](https://github.com/go-ruby-prime/docs). Start from the
